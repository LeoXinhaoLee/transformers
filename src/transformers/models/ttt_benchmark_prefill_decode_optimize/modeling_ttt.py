...
class TttBaseModule(nn.Module):
    ...
    def forward_chunk(
        ...
        is_last_in_chunk: Optional[bool] = False,
    ):
...
####### M1 Decode Module #######
def m1_prefill_chunk(states, inputs):
    W1_init = states['W1_states']
    XA_chunk, XB_chunk, XC_chunk, \
    coeff_chunk, coeff_chunk_last = inputs['XA'], inputs['XB'], inputs['XC'], inputs['coeff'], inputs['coeff_last']
    ...
    W1_init.sub_((coeff_chunk_last * XB_chunk).transpose(-1, -2) @ Z1)  # in-place update
...
####### M2 Decode Module #######
def m2_prefill_chunk(states, inputs):
    W1_init = states['W1_states']
    ...
    W1_init.sub_((coeff_chunk_last * XB_chunk).transpose(-1, -2) @ grad_l_wrt_Z1)  # in-place update init

    Attn2 = torch.tril(Z1_bar @ Z1.transpose(-1, -2))
    Z2_bar = Z1_bar @ W2_init - (coeff_chunk * Attn2) @ Z2
    W2_init.sub_((coeff_chunk_last * Z1).transpose(-1, -2) @ Z2)  # in-place update init
