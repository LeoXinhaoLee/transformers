...
from transformers.models.ttt_full_prefill_decode_optimize.micro_decode_modules import ttt_m1_triton_decode
...
def benchmark_decode(N, BS, NH, CS, HF, HF_prime, provider):
    ...
    if 'M1' in provider:
        ...
        b1_grad = torch.randn_like(b1) * 0.02
    elif 'M2' in provider:
        W1 = torch.randn(BS, NH, HF, HF_prime, device='cuda', dtype=input_dtype) * 0.02
        W1_grad = torch.randn_like(W1) * 0.02
        b1 = torch.randn(BS, NH, 1, HF_prime, device='cuda', dtype=input_dtype) * 0.02
        b1_grad = torch.randn_like(b1) * 0.02
        W2 = torch.randn(BS, NH, HF_prime, HF, device='cuda', dtype=input_dtype) * 0.02
        W2_grad = torch.randn_like(W2) * 0.02
        b2 = torch.randn(BS, NH, 1, HF, device='cuda', dtype=input_dtype) * 0.02
        b2_grad = torch.randn_like(b2) * 0.02
    else:
    ...
    if provider == 'M1 torch-native':
        ms, min_ms, max_ms = do_bench(lambda: loop(ttt_m1_decode),
                                      quantiles=quantiles, profile=args.profile)
    elif provider == 'M1 triton':
        ms, min_ms, max_ms = do_bench(lambda: loop(ttt_m1_triton_decode),
                                      quantiles=quantiles, profile=args.profile)
    elif provider == 'M1 triton CG':
